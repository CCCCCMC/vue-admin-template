<template>
  <div class="app-container">
    <el-table :data="tableData" style="width: 100%">
      ...
      <el-table-column label="供应商ID" width="180">
        <template slot-scope="scope">
          <el-popover trigger="hover" placement="top">
            <div slot="reference" class="name-wrapper">
              <el-tag size="medium">{{ scope.row.supplierId }}</el-tag>
            </div>
          </el-popover>
        </template>
      </el-table-column>
      <el-table-column label="订购日期" width="260">
        <template slot-scope="scope">
          <i class="el-icon-time"></i>
...
<script>
import { getPurchaseOrders, editPurchaseOrder, DeletePurchaseOrderById } from "@/api/SCM/PurchaseOrder";
...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      //跳转到添加页面，同时传递品牌id，方便在添加页面查询品牌信息，并显示
      this.$router.push("/HRM/add/");
    },
    changePage(pageNum) {
      getPurchaseOrders(pageNum).then((response) => {
        this.tableData = response.data.items.records;
      });
    },
  },
